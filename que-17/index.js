...
},
{
    ...
    githubUrl: "https://github.com/Fara-776",
...
async function Studentdata(githubUrl) {
    try {
...
async function displayStudents() {
    const studentCards = document.getElementsByClassName('student');

    for (let i = 0; i < students.length; i++) {
        const student = students[i];
        const githubData = await Studentdata(student.githubUrl);
        ...
        <p><b>Favorite IT Company:</b> ${student.favItCompany}</p>
        <a href=" ${student.githubUrl}"><b><u>Github URL:</u><b>${student.githubUrl}</a>
        <p><b>Email:</b> ${student.mailId}</p>
